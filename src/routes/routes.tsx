...
export const RouteContent = ({ pageProps }: IRenderedRoutes) => {
    ...
    switch (pageProp.type) {
        ...
        sectionContent.push(
          <Stack direction="row">
            <img
              ...
              src={`${import.meta.env.BASE_URL}src/editorial-contents/${pageProp.pageProp.path}`}
      ...
      case "project-card":
        sectionContent.push(
          <Stack>
            {isMobile ? (
              <ProjectCardCarrousel children={pageProp.pageProp} />
            ) : (
              <ProjectCard children={pageProp.pageProp} />
            )}
          </Stack>
        );
        break;
  ...
  return (
    <Stack>
      {headingVideoPath && (
        <video
          src={`${import.meta.env.BASE_URL}src/editorial-contents/${headingVideoPath}`}
          // controls
  ...
  for (const [lang, data] of Object.entries(editorials)) {
    if (Array.isArray(data.routes)) {
      for (const route of data.routes) {
        buildRoutes(route, lang.toLowerCase() as "fr" | "en");
      }
    }
  }
